<!--
  PermissionState — the "you can't see this" panel.
  Used when a capability/auth gate withholds a region (e.g. the inspector's
  Tasks panel when the requestor is not identifiable). Distinct from ErrorState:
  nothing failed; the region is intentionally withheld.
-->
<script lang="ts">
  interface Props {
    /** The headline — real copy. */
    title?: string;
    /** Why access is withheld / what is needed. */
    description?: string;
    /** Optional action label (e.g. "Sign in", "Enable capability"). */
    actionLabel?: string;
...
<div class="dy-state" role="note" data-testid="permission-state">
