...
if [ "${#smoke_scripts[@]}" -eq 0 ]; then
  ...
  # wall-clock. Each script is hermetic — its own mktemp/OS-assigned ports, or
  # a distinct fixed /tmp name — so they parallelise safely. Output is captured
  # per-script and replayed in stable (collection) order afterwards, so the log
  # reads identically to the old sequential run regardless of finish order.
  #
  ...
  echo "(running ${#smoke_scripts[@]} smoke scripts, ${jobs}-way parallel)"
