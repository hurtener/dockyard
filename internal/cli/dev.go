...
// in-process on a contract change, and supervises the Vite dev server (which
// owns Svelte HMR). Ctrl-C tears the whole tree down cleanly. Dockyard does
// not shell out to air or wgo — the watcher is embedded (RFC §9.2).
func newDevCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "dev",
		Short: "Run the project's dev loop — watch, regenerate, restart",
